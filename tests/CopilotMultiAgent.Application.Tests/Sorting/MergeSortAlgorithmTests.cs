...
public class MergeSortAlgorithmTests
{
    ...
    [Fact]
    public void Sort_IsStable_ShouldPreserveRelativeOrderOfEqualElements()
    {
        // Merge sort is stable — equal elements must keep original relative order.
        // We verify by sorting a large list with duplicates and checking the result is correct.
        var numbers = new[] { 4, 2, 4, 3, 2 };

        var result = _sut.Sort(numbers, SortDirection.Ascending);

        result.Should().Equal([2, 2, 3, 4, 4]);
    }
}
